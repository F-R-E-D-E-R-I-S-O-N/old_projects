...
file_name = input('Как назовем папку, куда все это сложим?\n')
...
loading = 100
file = open('D:\Новая папка\\'+ file_name +'.txt', 'a+', encoding='utf-8')
r = 0
os.system('cls||clear')
print(r*'X', (loading - r) * '_','loading:  ', r, '%', sep ='')
...
for title in titles:
    ...
    if counter > 1 and title not in repeat_list:
        ...
        file.write(f'<start>\nbook("{title}")   image: {titles_link(title)}\n')
        link = titles_link(title)
        for j in range(counter): # Генерирую повторяющиеся продукты и их вариации со ссылкой
            check += 1                                                                               # ██████
            file.write(f'         ▪ variant{generator_product(title, colors, sizes, check)}\n') 
            file.write(f'         image: {link}\n')
        ...
        check += 1  
        titled = f'<start>\n◦ book{generator_product(title, colors, sizes, check)}\nimage: {titles_link(title)}\n<end>\n\n\n'                                                                                 # ██████
        file.write(titled)
        if check % 10 == 0:
                r = int(check / 10)
                os.system('cls||clear')
